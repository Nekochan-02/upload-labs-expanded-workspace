...
* **Phase 2C-F20 / targeted regression plan**:
  ...
  * The `space` cap `200` UI/purchase smoke now passes: cap behavior and purchase flow continue past `100` with no visible error. Group movement across the old boundary remains required before RC. A future clean RC still requires its own clean-install/startup smoke check.

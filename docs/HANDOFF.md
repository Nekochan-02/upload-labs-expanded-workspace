...
* **Phase 2C-F14 / group resize expanded-bounds canary**:
  * **Status: `F14_CANARY_READY_FOR_USER_TEST`**
  ...
  * User must test the top-left edge path in a temporary state with only `0.2.21`, not save after failure, then provide `[F14]` logs. Group persistence, full regression, release integration, public-master push, Release/tag/Workshop operation, and v0.2.9 artifact changes remain deferred.
* **Phase 2A 検証状態**:

...
* **Phase 2C-F17 / right-side resize collapse targeted canary**:
  ...
  * `0.2.24` remains a local development canary only: `dist/Nekochan-ExpandedWorkspace-0.2.24.zip`, `22115` bytes, `15` files, ZIP root `mods-unpacked`, SHA-256 `942f67e0e0535b208a6ecc67d1d13cd9baf714035a8471dcdad55926373e7e7c`. F17 edge coverage is complete for its primary/secondary scope. The single next action is to request approval for a plan-only F12 group-persistence re-entry gate; do not resume F12 itself. Full regression, release integration, public push, Release/tag/Workshop operation, and v0.2.9 artifact operations remain deferred.
* **Phase 2A 検証状態**:
